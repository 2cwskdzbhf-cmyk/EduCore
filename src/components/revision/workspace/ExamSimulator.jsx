...
const BG = 'linear-gradient(135deg, #EDE8F5 0%, #c8d4f5 60%, #b0c0f0 100%)';
...
function TestSetupPanel({ notebook, allSources, onStart }) {
  ...
  const activeBtn = { background: 'linear-gradient(135deg, #7091E6, #3D52A0)', color: '#fff', borderRadius: '12px', fontWeight: 700, border: 'none', cursor: 'pointer', padding: '10px 0' };
  const inactiveBtn = { background: 'rgba(255,255,255,0.45)', border: '1px solid rgba(255,255,255,0.5)', color: '#3D52A0', borderRadius: '12px', fontWeight: 600, cursor: 'pointer', padding: '10px 0' };

  return (
    <div className="flex flex-col items-center justify-center h-full p-6 overflow-y-auto" style={{ background: BG }}>
      <motion.div
        ...
        <div className="space-y-5">
          ...
          <div>
            ...
            <div className="flex gap-2 mb-3">
              <button onClick={() => setUseCustomTopic(false)} className="flex-1 text-sm" style={!useCustomTopic ? activeBtn : inactiveBtn}>From My Sources</button>
              <button onClick={() => setUseCustomTopic(true)} className="flex-1 text-sm" style={useCustomTopic ? activeBtn : inactiveBtn}>Custom Topic</button>
            </div>
            {useCustomTopic ? (
              <input
                value={customTopic}
              ...
              <p className="text-sm p-3 rounded-xl" style={{ background: 'rgba(220,55,55,0.08)', color: '#c0392b' }}>
                No sources found. Add sources or switch to Custom Topic.
              </p>
            ) : (
            ...
            <div className="space-y-2">
              {TEST_MODES.map(m => (
                <button key={m.id} onClick={() => setTestMode(m.id)}
                  ...
                  style={{
                    background: testMode === m.id ? 'rgba(112,145,230,0.15)' : 'rgba(255,255,255,0.4)',
                  ...
                  {testMode === m.id && <CheckCircle className="w-4 h-4 flex-shrink-0" style={{ color: '#7091E6' }} />}
                </button>
              ))}
            </div>
          </div>
          ...
          <button
            onClick={() => onStart({ numQuestions: parseInt(numQuestions), difficulty, questionType, testMode, customTopic: useCustomTopic ? customTopic.trim() : null, timeLimitSeconds: parseInt(timeLimit) * 60 })}
            disabled={!canStart}
...
function GeneratingScreen({ numQuestions }) {
  return (
    <div className="flex flex-col items-center justify-center h-full gap-5" style={{ background: BG }}>
      ...
      <p className="text-xs" style={{ color: 'rgba(61,82,160,0.5)' }}>This may take up to 30 seconds</p>
    </div>
  );
}
...
function AntiCheatWarning({ warnings, maxWarnings, onDismiss }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-center justify-center"
      style={{ background: 'rgba(220,55,55,0.35)', backdropFilter: 'blur(8px)' }}>
      <motion.div initial={{ scale: 0.85, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
        ...
        <p className="text-gray-600 text-sm mb-6">
          {isFinal ? 'You have exceeded the limit. The test will now end.' : 'You left the exam window. Return immediately.'}
        </p>
        <button onClick={onDismiss} className="w-full py-3 rounded-xl font-bold text-white text-sm"
          style={{ background: isFinal ? '#dc2626' : 'linear-gradient(135deg, #7091E6, #3D52A0)' }}>
          {isFinal ? 'End Test' : 'Return to Test'}
        </button>
      </motion.div>
    </div>,
    document.body
...
function ReviewScreen({ questions, answers, flagged, onGoTo, onSubmit, onBack }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9998] flex flex-col" style={{ background: BG }}>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-2xl mx-auto space-y-3">
          {questions.map((q, i) => {
            ...
            return (
              <button key={i} onClick={() => onGoTo(i)} className="w-full text-left p-4 rounded-2xl transition-all"
                style={{ background: 'rgba(255,255,255,0.55)', border: isFlagged ? '1.5px solid #fbbf24' : isAnswered ? '1.5px solid rgba(16,185,129,0.4)' : '1.5px solid rgba(220,55,55,0.3)' }}>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 font-bold text-sm"
                    style={{ background: isFlagged ? '#fef3c7' : isAnswered ? 'rgba(16,185,129,0.15)' : 'rgba(220,55,55,0.1)', color: isFlagged ? '#d97706' : isAnswered ? '#059669' : '#dc2626' }}>
                    {i + 1}
                  </div>
      ...
      <div className="flex-shrink-0 p-6 flex items-center gap-3"
        style={{ background: 'rgba(255,255,255,0.45)', backdropFilter: 'blur(20px)', borderTop: '1px solid rgba(255,255,255,0.4)' }}>
        ...
        <button onClick={onBack} className="px-5 py-2.5 rounded-xl font-semibold text-sm"
          style={{ background: 'rgba(255,255,255,0.55)', border: '1px solid rgba(255,255,255,0.4)', color: '#3D52A0' }}>
          Continue
        </button>
        <button onClick={onSubmit} className="px-6 py-2.5 rounded-xl font-bold text-sm text-white"
          style={{ background: 'linear-gradient(135deg, #7091E6, #3D52A0)' }}>
          Submit Test
        </button>
      </div>
...
// ─── Test Mode ────────────────────────────────────────────────────────────────
function TestMode({ questions, settings, notebook, onFinish }) {
  ...
  // Delay anti-cheat a few seconds so fullscreen request doesn't immediately fire blur
  ...
  // Enter fullscreen via portal — this IS document.body level
  useEffect(() => {
    const el = document.documentElement;
    if (el.requestFullscreen) el.requestFullscreen().catch(() => {});
    // Delay anti-cheat activation to avoid false trigger from fullscreen transition
    const t = setTimeout(() => { antiCheatActiveRef.current = true; }, 2500);
    return () => {
      clearTimeout(t);
  ...
  const triggerWarning = useCallback(() => {
    if (endedRef.current || !antiCheatActiveRef.current) return;
  ...
  const doFinish = useCallback((autoEnded, suddenDeath) => {
  ...
  }, [answers, flagged, onFinish, settings.timeLimitSeconds]);
  ...
  const handleAnswer = (value) => {
    setAnswers(prev => {
      ...
      if (settings.testMode === 'sudden_death') {
        ...
        if (!isCorrect) setTimeout(() => doFinish(false, true), 700);
      }
      return next;
    });
  };
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-6 py-3"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 px-4 py-2 rounded-xl font-black text-lg"
            style={{ background: 'rgba(255,255,255,0.65)', border: `2px solid ${timerColor}`, color: timerColor }}>
          ...
          <button onClick={() => setShowNavigator(v => !v)}
            ...
            style={{ background: showNavigator ? 'rgba(112,145,230,0.2)' : 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.4)', color: '#3D52A0' }}>
      ...
      <AnimatePresence>
        {showNavigator && (
          <motion.div initial={{ y: -8, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -8, opacity: 0 }}
            ...
            style={{ background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.5)', boxShadow: '0 8px 32px rgba(61,82,160,0.15)' }}>
                ...
                return (
                  <button key={i} onClick={() => { if (canNav) { setCurrentIndex(i); setShowNavigator(false); } }}
                    ...
                    style={{
                      ...
                      cursor: !canNav ? 'not-allowed' : 'pointer',
                    }}>
                    {i + 1}
                  </button>
                );
      ...
      <div className="flex-1 flex items-center justify-center p-6 overflow-y-auto">
        <AnimatePresence mode="wait">
          <motion.div key={currentIndex} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}
            transition={{ duration: 0.15 }} className="w-full max-w-2xl rounded-3xl p-8" style={GLASS}>
            ...
            <div className="flex items-start justify-between mb-6">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-3 flex-wrap">
                  ...
                  <span className="px-3 py-1 rounded-full text-xs font-semibold capitalize" style={{ background: 'rgba(134,151,196,0.12)', color: '#8697C4' }}>
                    {(q.type || 'multiple_choice').replace('_', ' ')}
                  </span>
                  {flagged.has(currentIndex) && (
                    <span className="px-3 py-1 rounded-full text-xs font-bold" style={{ background: '#fef3c7', color: '#d97706' }}>Flagged</span>
                  )}
                </div>
                <h3 className="text-lg font-bold leading-relaxed" style={{ color: '#3D52A0' }}>{q.question}</h3>
              </div>
              <button onClick={toggleFlag} className="ml-4 p-2.5 rounded-xl transition-all flex-shrink-0"
                style={{
            ...
            {/* Answer options — NO correct/wrong feedback shown during test */}
            {q.type === 'short_answer' ? (
              <textarea value={answers[currentIndex] || ''} onChange={e => handleAnswer(e.target.value)}
              ...
              <div className="grid grid-cols-2 gap-3">
                {[0, 1].map(i => (
                  <button key={i} onClick={() => handleAnswer(i)}
                    ...
                    style={{
                      background: answers[currentIndex] === i ? 'linear-gradient(135deg, #7091E6, #3D52A0)' : 'rgba(255,255,255,0.55)',
              ...
              <div className="space-y-3">
                {(q.options || []).map((opt, i) => (
                  <button key={i} onClick={() => handleAnswer(i)}
                    ...
                    style={{
                      background: answers[currentIndex] === i ? 'rgba(112,145,230,0.18)' : 'rgba(255,255,255,0.55)',
            ...
            <div className="flex justify-between items-center mt-8">
              <button onClick={goPrev} disabled={currentIndex === 0 || settings.testMode === 'hard'}
  ...
  return createPortal(testContent, document.body);
}
...
function ResultsScreen({ questions, answers, settings, timeUsed, suddenDeath, autoEnded, notebook, user, onResourceCreated, onClose }) {
      ...
      try {
        const result = await base44.integrations.Core.InvokeLLM({
          ...
          response_json_schema: {
            ...
            required: ['strengths', 'weaknesses', 'next_steps', 'overall_feedback'],
  ...
  const content = (
    <div className="fixed inset-0 z-[9996] flex flex-col overflow-y-auto" style={{ background: BG }}>
      <div className="max-w-2xl mx-auto w-full p-6 space-y-6 pb-16">

        {/* Score card */}
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          className="rounded-3xl p-8 text-center" style={GLASS}>
          {suddenDeath && <p className="font-bold text-sm mb-3" style={{ color: '#ef4444' }}>Sudden Death — Test Ended Early</p>}
          {autoEnded && <p className="font-bold text-sm mb-3" style={{ color: '#f59e0b' }}>Test ended due to anti-cheat violations</p>}
          ...
          {loadingBreakdown ? (
            ...
            <div className="space-y-4">
              {breakdown.overall_feedback && (
                <p className="text-sm leading-relaxed p-3 rounded-xl" style={{ background: 'rgba(255,255,255,0.45)', color: '#3D52A0' }}>
                  {breakdown.overall_feedback}
                </p>
              )}
        ...
        <div className="flex gap-3">
          <button onClick={exportResults} disabled={exported}
            ...
            style={{ background: exported ? '#10b981' : 'rgba(255,255,255,0.55)', border: exported ? 'none' : '1px solid rgba(255,255,255,0.4)', color: exported ? '#fff' : '#3D52A0' }}>
            {exported ? <><CheckCircle className="w-4 h-4" /> Saved to Items</> : <><Download className="w-4 h-4" /> Export Results</>}
          </button>
          <button onClick={onClose} className="flex-1 py-3 rounded-xl font-bold text-sm text-white"
            style={{ background: 'linear-gradient(135deg, #7091E6, #3D52A0)' }}>
            Done
          </button>
        </div>
...
export default function ExamSimulator({ notebook, user, allSources, onResourceCreated }) {
  ...
  const handleStart = async (cfg) => {
    ...
    const ctx = cfg.customTopic
      ? `Topic: ${cfg.customTopic}. Generate general knowledge questions on this topic.`
      : allSources.filter(s => s.content_text).map(s => `### ${s.name}\n${s.content_text.slice(0, 6000)}`).join('\n\n').slice(0, 14000);

    const typeInstructions = {
      multiple_choice: 'All questions must be type="multiple_choice" with 4 options and a correct_index (0-3).',
      true_false: 'All questions must be type="true_false" with options=["True","False"] and correct_index (0=True, 1=False).',
      short_answer: 'All questions must be type="short_answer" with a correct_answer field (brief 1-2 sentence answer).',
      mixed: 'Mix: roughly half type="multiple_choice" (4 options, correct_index), some type="true_false" (options=["True","False"], correct_index), some type="short_answer" (correct_answer).',
    };

    const diffInstructions = {
      easy: 'Easy recall-based questions suitable for beginners.',
      medium: 'Medium difficulty, testing understanding and application.',
      hard: 'Hard analysis and evaluation questions.',
      'exam-level': 'Exam-level rigor with precise technical language.',
    };

    try {
      const qs = await base44.integrations.Core.InvokeLLM({
        prompt: `Generate exactly ${cfg.numQuestions} exam questions.
${diffInstructions[cfg.difficulty] || ''}
${typeInstructions[cfg.questionType] || ''}
Always include: question (string), type (string), explanation (string, 1-2 sentences why the answer is correct).
For multiple_choice: also include options (array of 4 strings) and correct_index (number 0-3).
For true_false: also include options=["True","False"] and correct_index (0 or 1).
For short_answer: also include correct_answer (string).
